...
function hookSocket(){
  ...
  if(!window.__ssHk){window.__ssHk=true;
    socket.on('screen_share_started',async({username,socketId})=>{window.showToast?.(`🖥 ${username} داره صفحه‌ش رو نشون میده`);const pc=await window.createScreenPC(socketId,false);socket.emit('screen_request',{to:socketId});});
    socket.on('screen_share_stopped',()=>{document.getElementById('_ssOv')?.remove();window.showToast?.('🖥 اشتراک صفحه تموم شد');});
    socket.on('screen_request',async({from})=>{if(!window.isSharing||!window.screenStream)return;await window.createScreenPC(from,true);});
    socket.on('screen_offer',async({from,offer})=>{const pc=window.screenPCs?.[from];if(!pc)return;await pc.setRemoteDescription(offer);const ans=await pc.createAnswer();await pc.setLocalDescription(ans);socket.emit('screen_answer',{to:from,answer:ans});});
    socket.on('screen_answer',async({from,answer})=>{const pc=window.screenPCs?.[from];if(!pc)return;await pc.setRemoteDescription(answer);});
